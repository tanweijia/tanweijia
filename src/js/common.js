...
var checkReg = {
	...
	psweasy: function(str) { //6-16位首字母开头
...
function autoCar(urlphp,fnyes,fnNo,fnunlogin){//传参 首页url="" 其他页面url=../
    var totalqty=0;
...
function creatAddMycar(arr,urltype){//数据渲染函数封装
    $('#cartListView ul').html('');
    var totalprice = 0
...
function creatAddCarSide(arr,addqty,totalqty){//数据渲染函数封装
    $('#item_add').html("");
    var res = `<li class="b0">
                    <div class="pic">
                        <img src="${arr.imgurl1}">
    ...
    $('.new_car').animate({right:"52px",display:"block"},1500,function(){$('.car_buy').removeClass('move');});
